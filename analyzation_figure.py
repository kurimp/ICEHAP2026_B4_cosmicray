...
def fitting(df):
  ...
  popt02, pcov02 = curve_fit(model02, df['Mean_CosTheta_CosSquaredTheta'], df["Mean_Flux"], sigma = df["Std_Flux"], p0=[0.005, 2], maxfev=50000, absolute_sigma=True)
  return [popt01, popt02], pcov01, pcov02

popt, pcov01, pcov02 = fitting(df_res)
popt_weighted, pcov01_weighted, pcov02_weighted = fitting(df_res_weighted)

a01 = float(f"{popt[0][0]}")
b01 = float(f"{popt[0][1]}")
n01 = float(f"{popt[0][2]:}")

b02 = float(f"{popt[1][0]:}")
n02 = float(f"{popt[1][1]:}")
...
b02_weighted = float(f"{popt_weighted[1][0]:}")
n02_weighted = float(f"{popt_weighted[1][1]:}")
...
def test(df, popt):
  ...
  degrees_of_freedom = [N - P[0], N - P[1]]
...
#効率反映前後のプロット
#同じ位置のデータを統合したプロット
#効率反映、データ統合後のプロットとフィット曲線

maintitle = "Flux vs. Average Cosine of Zenith Angle"
title_list = ["(before weighted)", "(weighted)", "(integrated)", "(fitted)"]
title = [maintitle + title_list[i] for i in range(len(title_list))]

df_list = [df_res_base, df_res_base_weighted, df_res_weighted, df_res_weighted]
...
for i in range(len(title)):
  ...
  if title_list[i] == "(before weighted)":
    v_a01 = a01
    v_b01 = b01
    v_n01 = n01
    v_b02 = b02
  ...
  if title_list[i] == "(fitted)":
    ax.plot(np.linspace(0, 1, 10000), model01(np.linspace(0, 1, 10000), v_a01, v_b01, v_n01), linewidth=0.5,label=f'Fitted Curve①')
    ax.plot(np.linspace(0, 1, 10000), model02(np.linspace(0, 1, 10000), v_b02, v_n02), linewidth=0.5, label=f'Fitted Curve②')
  
    print("----Fitted Curve①----")
    print(f"Flux=({v_a01:.2e}±{v_err01[0]:.2e}) + ({v_b01:.2e}±{v_err01[1]:.2e})(cosθ)^({v_n01:.2e}±{v_err01[2]:.2e}) (reduced χ^2:{v_reduced_chi_squared01:.2f})")
    print("----Fitted Curve②----")
    print(f"Flux=({v_b02:.2e}±{v_err02[0]:.2e})(cosθ)^({v_n02:.2e}±{v_err02[1]:.2e}) (reduced χ^2:{v_reduced_chi_squared02:.2f})")
  ...
  plt.tight_layout()
  plt.savefig("./" + title_list[i] + ".png", dpi = 300)
